...
class CreateResultsView(Query):
    ...
    def statement(
        ...
        FROM (
            SELECT 
                bgp.ip_asn, 
                results.probe_dst_addr,
                results.probe_src_port, 
                results.reply_src_addr, 
                results.probe_ttl, 
                if(results.asn!='-1', 
                    results.asn, 
                    if(results.reply_src_addr!='::',arrayStringConcat(arraySlice(splitByChar('.', toString(probe_dst_addr)), 1, 2), '.') || '.0.0','-1'))
                AS asn,
                bgp.len,
            ...
            LEFT JOIN {bgp_table(measurement_id)} bgp
            ON toIPv6(arrayStringConcat(arraySlice(splitByChar('.', toString(results.probe_dst_addr)), 1, 3), '.') || '.0') = tupleElement(bgp.ipv6_range,1)
        ) AS t1
        WHERE t1.row_num = 1
        """