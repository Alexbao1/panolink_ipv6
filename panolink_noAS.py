...
parser.add_argument('--rate', type=int, required=False, default=10000, help='The probing rate')
...
prefixes = [(target_prefix, "udp", range(2, 20), 6)]
total_probes = 0
n_probes_list=[]
n_links_list=[]
bootstrap_rounds = 1
...
if __name__ == "__main__": 
    ...
    with ClickHouseClient(**credentials) as client:
        ...
        true_link = client.json(f"SELECT count(*) as link_cnt FROM (SELECT DISTINCT near_addr,far_addr FROM {links_table(measurement_id)} WHERE near_addr != toIPv6('::') AND far_addr != toIPv6('::'))")
        true_link = true_link[0]['link_cnt']
        time_used = client.json(f"SELECT max(capture_timestamp)-min(capture_timestamp) as total_time FROM {results_table(measurement_id)}")
        time_used = time_used[0]['total_time']
        ...
        with open('result/tight_output.csv', 'a') as f:
            # Use the print() function's 'file' parameter to write to the file
            print(f"{item},",end='', file=f)
            print('panolink,', end='', file=f)
