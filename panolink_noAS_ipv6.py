...
from diamond_miner.insert import insert_mda_probe_counts, insert_probe_counts, improved_bootstrap
...
from diamond_miner.utilities import get_result_path, run_add_round
...
parser.add_argument('--max_next_hop', type=int, required=False, default=1000, help='The maximum number of next hops to consider.')
parser.add_argument('--rate', type=int, required=False, default=100000, help='The probing rate')
parser.add_argument('--item', type=str, required=False, default='test', help='The item name')
args = parser.parse_args()
target_prefix = args.target_prefix
max_next_hop = args.max_next_hop
rate = args.rate
item = args.item
...
if __name__ == "__main__": 
    ...
    with ClickHouseClient(**credentials) as client:
        ...
        for round_ in range(1, 24):
            ...
            if round_ == 1:
                ...
                insert_probe_counts(
                    ...
                    round_=round_,
                    prefixes=[(target_prefix, "udp", range(2, 21), 1)],
                )

            elif round_ <=6:
                replace_round_column(get_result_path(results_filepath, round_-1), round_-1)
                ...
                if round_<=100:
                    insert_mda_probe_counts(
                        ...
                        max_next_hop=max_next_hop,
                    )
                    insert_weight(client, measurement_id,round_,max_next_hop=max_next_hop)
                    insert_as_probe(client, measurement_id,round_)
        ...
        true_link = client.json(f"SELECT count(*) as cnt FROM (SELECT DISTINCT near_addr,far_addr FROM {links_table(measurement_id)} WHERE near_addr != toIPv6('::') AND far_addr != toIPv6('::'))")
        true_link = true_link[0]['cnt']
        time_used = client.json(f"SELECT max(capture_timestamp)-min(capture_timestamp) as time FROM {results_table(measurement_id)}")
        time_used = time_used[0]['time']
        probing_time=client.json(f"SELECT count(DISTINCT capture_timestamp) as probing_time FROM {results_table(measurement_id)}")
        probing_time = probing_time[0]['probing_time']
        ...
        with open('result/output.txt', 'a') as f:
            ...
            print('panolink_noAS_ipv6', file=f)
        ...
        with open('result/ipv6/tight_output', 'a') as f:
            # Use the print() function's 'file' parameter to write to the file
            print(f"{item},",end='', file=f)
            print('panolink_ipv6,', end='', file=f)

            print(f"{target_prefix},", end='', file=f)
            print(f"{rate},", end='', file=f)

            print(f"{total_probes},", end='', file=f)
            print(f"{len(links)},", end='', file=f)
            print(f"{true_link},", end='', file=f)
            
            print(f"{time_used},", end='', file=f)
            print(f"{probing_time},", end='', file=f)

            print(f"{measurement_id.replace('-', '_')},", end='',file=f)
            print(f"{datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S')}", file=f)

        with open('stage2/temp_measurement_id.txt', 'w') as f:
