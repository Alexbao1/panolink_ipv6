...
from diamond_miner.defaults import DEFAULT_PREFIX_SIZE_V4,DEFAULT_PREFIX_SIZE_V6, DEFAULT_PREFIX_LEN_V6
...
class RandomFlowMapper:
    ...
    def offset(self, flow_id: int, prefix: int, prefix_len: int=DEFAULT_PREFIX_LEN_V6) -> tuple[int, int]:
        if flow_id < self.prefix_size:
            perm = self.permutations[prefix % len(self.permutations)]
            first = perm[flow_id]
            second = perm[flow_id+1]
            _offset = (first << 64) + second
            mask = self.base >> prefix_len
            _offset = _offset & mask
            return _offset, 0
        else:
            return self.prefix_size - 1, flow_id - self.prefix_size + 1