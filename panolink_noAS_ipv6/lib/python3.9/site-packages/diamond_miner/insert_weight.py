...
def insert_weight(client: ClickHouseClient, 
                    ...
                    max_next_hop: int = 100):
    
    changeable_weight_query = f"""
        {max_next_hop} as max_next_hop
        , max_next_hop_limit AS (
            SELECT
                near_addr,
                max_next_hop/arrayMax([max_next_hop,count(DISTINCT far_addr)]) as near_addr_weight
        ...
        , link AS (
            ...
            FROM (
                SELECT 
                    groupUniqArray(probe_dst_prefix) AS list_probe_dst_prefix, 
                    near_ttl, 
                    far_ttl, 
                    near_addr, 
                    far_addr, 
                    max(m.near_addr_weight)/length(list_probe_dst_prefix) as contribution 
                FROM {links_table(measurement_id)}
                LEFT JOIN max_next_hop_limit m
                USING near_addr
