...
from diamond_miner.queries.query import Query,ProbesQuery ,probes_table,pre_probes_table, results_table
...
def improved_bootstrap(
    ...
    INSERT INTO {probes_table(measurement_id)}
    WITH r AS (
        SELECT *, toIPv6(IPv6NumToString( toFixedString( unbin( CONCAT( SUBSTRING(bin(IPv6StringToNum(toString(probe_dst_addr))), 1, LENGTH( bin(IPv6StringToNum(toString(probe_dst_addr))) ) - {128-DEFAULT_PREFIX_LEN_V6}), REPEAT('0', {128-DEFAULT_PREFIX_LEN_V6}))) ,16 ) )) as probe_dst_prefix 
        FROM {results_table(measurement_id)}
    )
    , p AS (
        SELECT * FROM {probes_table(measurement_id)}
        WHERE round = {round_}
    )
    SELECT probe_protocol, probe_dst_prefix, probe_ttl, cumulative_probes+1 as cumulative_probes, round+1 as round FROM p
    LEFT JOIN r
    ON p.probe_dst_prefix=r.probe_dst_prefix
    AND p.probe_ttl=r.probe_ttl
    WHERE r.probe_dst_prefix =toIPv6('::') AND r.probe_ttl =0            
    """)
    
def insert_last_round_probe(
    client: ClickHouseClient,
    measurement_id: str,
    ...
    client.json(f"""
    INSERT INTO {probes_table(measurement_id)}
    SELECT *, 1 AS round FROM (
        SELECT probe_protocol, probe_dst_prefix, probe_ttl, cumulative_probes FROM {probes_table(last_measurement_id)}
...
def insert_mda_probe_counts(
    ...
    # )
    client.json(query.statement(measurement_id))
