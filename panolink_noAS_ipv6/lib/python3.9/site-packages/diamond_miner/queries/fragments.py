...
def cut_ipv6_bit(column: str, prefix_len_v6: int) -> str:
    bin_ipv6 = f"bin(IPv6StringToNum(toString({column})))"
    return f"""
    toIPv6( 
        IPv6NumToString( 
            toFixedString( 
            unbin( 
                CONCAT(
                    SUBSTRING({bin_ipv6}, 1, LENGTH( {bin_ipv6} ) - {128-prefix_len_v6}), REPEAT('0', {128-prefix_len_v6})
                )
            )
            ,16
        )
