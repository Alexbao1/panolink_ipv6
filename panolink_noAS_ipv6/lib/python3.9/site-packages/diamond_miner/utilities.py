...
def run_add_round(path):
    subprocess.call("/bin/bash /root/panolink/add_round.sh " + path,shell=True)

def run_process_dminer(dminer_path):
    subprocess.call("/bin/bash /root/panolink/process_dminer.sh " + dminer_path,shell=True)
...
def run_bdrmapit(client,measurement_id,round):
    print("???")
    subprocess.run(f'printf "\\n\\"/root/panolink/results.csv_{round}_processed.warts\\"" >> /root/panolink/wartsfile_panolink', shell=True, check=True)
    print("!!!") 
    subprocess.call("sudo /root/anaconda3/bin/conda run -n bdrmapit bdrmapit json -c /root/bdrmapit/config.json -s /root/panolink/output.db ",shell=True)
    subprocess.call("sudo docker cp /root/panolink/output.db  913a5271e07f:/var/lib/clickhouse/user_files ",shell=True)
    # client.json(f"DROP TABLE {results_table(measurement_id)}")
    # client.json(f"DROP DICTIONARY {lookup_dict(measurement_id)}")
